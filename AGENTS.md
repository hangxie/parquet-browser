...
- Keep source files cohesive and prefer small files: aim to keep non-test Go source files under roughly 300 LOC, splitting into focused units when that improves clarity rather than treating it as a hard limit. Test files may exceed this when table coverage or fixtures make that clearer than splitting. When touching existing large files, avoid making them larger unless the change is tightly scoped; use a new focused file when that improves clarity.
...
- Do not add AI/agent attribution or provenance metadata in repo artifacts. Commit messages, PRs, issues, and documents must not mention AI assistants or agents, or include generation/provenance metadata such as `Co-Authored-By`, `Generated by ...`, `Created with ...`, or similar.
- Use `origin` for all pushes and GitHub operations unless explicitly instructed otherwise.
